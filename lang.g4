 //Baseado no python3-parser (https://github.com/bkiers/python3-parser) por Bart Kiers, bart@big-o.nl
 //<Licença do MIT aqui>
...
/// typedArgsList: (typedFunctionParam ['=' test] (',' typedFunctionParam ['=' test])* [','
...
typedFunctionParam
 : (paramPassage)? dataType (OPEN_BRACK CLOSE_BRACK)* INVISIBLE? NAME
 ;
...
/* IMPORT (usando <lista de nomes>) */
/// importStatement: import_name | import_from
importStatement
 : IMPORT nameList SEMI_COLON? NEWLINE
 ;
...
simpleStatement
 : smallStatement ( SEMI_COLON smallStatement )* SEMI_COLON? NEWLINE
 ;

/* SUBTIPOS DE DECLARAÇÃO */
/// smallStatement: (expressionStatement | del_stmt | pass_stmt | flowStatement |
///              importStatement | global_stmt | nonlocal_stmt | assert_stmt)
smallStatement
...
/* ENUMERAÇÕES */
enumSpecifier
 :   ENUM NAME? OPEN_BRACE enumeratorList CLOSE_BRACE
 ;
...
returnStatement
 : RETURN testOrExpressionList?
 ;

/// nameList: dotted_as_name (',' dotted_as_name)*
nameList
 : NAME ( COMMA NAME )*
 ;
...
/// forStatement: 'for' expressionList 'in' testList ':' block ['else' ':' block]
forStatement
 : FOR nameList ASSIGN rangeList (STEP expressionList)? COLON block
 ;
...
/// stringprefix    ::=  "r" | "R"
STRING_LITERAL
 : SHORT_STRING {self.text = self.text[1:-1]}
 ;
